...
(def test-connection derby)

(try
  (db/db-do-commands test-connection "DROP TABLE users")
  (catch Exception e))

(db/db-do-commands test-connection
  (db/create-table-ddl
    :users
...
(facts "sql-db"
  (let [users (wrap-with-honey (sql-db (constantly test-connection)) :users)]
